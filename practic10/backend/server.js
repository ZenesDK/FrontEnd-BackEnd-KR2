...
// Хранилище валидных refresh-токенов (в реальном проекте использовать БД)
...
// ==================== JWT MIDDLEWARE (для access-токена) ====================
...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: 'API с JWT аутентификацией и refresh-токенами',
    },
    servers: [{ url: `http://localhost:${port}`, description: 'Локальный сервер' }],
    tags: [
      { name: 'Auth', description: 'Регистрация, вход, обновление токенов' },
      { name: 'Products', description: 'Управление товарами (требуется accessToken)' }
    ],
    components: {
      securitySchemes: {
        bearerAuth: {
...
app.post("/api/auth/refresh", (req, res) => {
  ...
  try {
    ...
    // Если токен не прошел верификацию, удаляем его из хранилища (он мог быть скомпрометирован)
    refreshTokens.delete(refreshToken);
 ...
 *               $ref: '#/components/schemas/UserResponse'
 *       401:
 *         description: Не авторизован (отсутствует или невалидный access-токен)
 *       404:
